...
def payload_for(spec,prompt):
    ...
    for p in spec.get('parameters',[]):
        ...
        elif lname in {'system','system_prompt'}: payload[name]='Evidence-aware language and culture analysis. CLAIM<=EVIDENCE; interpretation is not fact.'
...
ROLE=os.environ.get('ROLE','UNKNOWN_ROLE'); QUESTION=os.environ.get('QUESTION','Analyze language and culture transfer rigorously.'); MODEL=os.environ.get('MODEL','huggingface-projects/llama-3.2-3B-Instruct')
prompt=f'''You are role {ROLE} in CEREBRON OMEGA Farm 29 Language Culture.\nQuestion: {QUESTION}\nRules: CLAIM<=EVIDENCE; DESCRIPTION!=VALUE_JUDGMENT; INTERPRETATION!=FACT; CORPUS!=POPULATION; CONTEXT_EXPLICIT; PROVENANCE_REQUIRED; CULTURAL_VARIATION_EXPLICIT; UNKNOWN_REMAINS_UNKNOWN. Return a structured evidence-aware analysis with assumptions, ambiguities, cultural context, counterexamples, uncertainty, verification needs, and transfer limits.'''
ok,text,meta=invoke(MODEL,prompt)
record={'farm':29,'domain':'language-culture','role':ROLE,'question':QUESTION,'model':MODEL,'status':'UNREVIEWED_EXTERNAL_AGENT_OUTPUT' if ok else 'EXTERNAL_INFERENCE_FAILED','inference_success':bool(ok),'timestamp_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'output':text if ok else None,'error':None if ok else meta.get('error'),'meta':meta}
OUT=pathlib.Path('artifacts'); OUT.mkdir(exist_ok=True); (OUT/f'{ROLE}.json').write_text(json.dumps(record,ensure_ascii=False,indent=2),encoding='utf-8'); print(json.dumps({'role':ROLE,'inference_success':bool(ok),'model':MODEL}))
